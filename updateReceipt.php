...
include "config.php";
    ...
    if (isset($_POST['update'])) 
    {
        ...
        $sql = "UPDATE `receipts` SET `Invoice No`='$invoiceno', `Receipt Date`='$recdate', `Vehicle Reg`= '$regno' ,`TRN`='$trn', 
	`Days Paid For`='$dayspaid', `Days Outstanding`= '$daysout' ,`Total Amount`='$total', `Amount Paid`='$ampaid', `Balance Remaining`= '$balance' WHERE `Invoice No`='$invoiceno'"; 
...
if (isset($_GET['id'])) 
{
    ...
    {        
        while ($row = $result->fetch_assoc()) 
        {
            ...
            $recdate = $row['Receipt Date'];
            $regno = $row['Vehicle Reg No'];
            $trn  = $row['TRN'];
	    $dayspaid = $row['Days Paid For'];
	    $daysout = $row['Days Outstanding'];
	    $total = $row['Total Amount'];
	    $ampaid = $row['Amount Paid'];
	    $balance = $row['Balance Remaining'];
        } 

    ?>
            <title>Update - Receipt Details</title>
            <style>
